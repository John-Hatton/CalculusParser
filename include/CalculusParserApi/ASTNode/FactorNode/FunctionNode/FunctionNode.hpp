...
#ifndef CALCULUSPARSER_FUNCTIONNODE_HPP
#define CALCULUSPARSER_FUNCTIONNODE_HPP 
...
FunctionNode::FunctionNode(Token token, std::string name)
        : FactorNode(std::move(token), FactorType::FUNCTION, token), name(std::move(name)) {}
...
#endif //CALCULUSPARSER_FUNCTIONNODE_HPP
